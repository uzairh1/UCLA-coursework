...
EVENT_WINDOW_HOURS = 48  # <-- HARD EVENT WINDOW
...
def main():
    ...
    # ----------------------------
    # BUILD SUMMARY (48h EVENT WINDOW + CHRONOLOGY)
    # ----------------------------
    ...
    for fire, hits in timeline.items():
        ...
        for stage in ("detection", "notification", "preparation", "evacuation"):
            ...
            filtered = [
                h for h in items
                if parse_time(h.published_at) >= earliest_allowed
            ]
            if filtered:
                items = filtered
            ...
            best[stage] = chosen
            earliest_allowed = parse_time(chosen.published_at)
