...
bool identicalValuesTogether(const string array[], int n)
{
	...
	for (int i = 0; i <= n - 1; i++)
	{
		for (int j = 0; j <= n - 1; j++)
		{
			if (array[i] == array[j]) // check element at array[i] for equality against ALL elements of the array.
			{
				counter++;
			}
		}
	...
	if ((counter - n) / 2 == 0)
		// since element at array[i] is checked for equality against ALL elements of the array,
		// including array[i] itself, n should be subtracted from counter to get the double the true number of duplicates. 
